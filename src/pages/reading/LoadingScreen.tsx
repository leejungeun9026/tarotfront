...
function LoadingScreen({
  ...
  return (
    <div
      className="LoadingScreen w-full max-w-3xl fixed z-999 left-1/2 top-0 -translate-x-1/2 flex justify-center items-center bg-violet-50"
      ...
      <div className="flex flex-col items-center gap-8">
        ...
        <div className="flex flex-col justify-center items-center gap-2">
          ...
          <div className="w-auto max-w-40 rounded-md shadow-md border overflow-hidden">
            <img
              src={getCardImg(viewIntervalCard.id) ?? undefined}
              alt={viewIntervalCard.nameEn}
              className={`${viewIntervalCard.reverse && "rotate-180"}`}
            />
          </div>
          <Card className="py-3 rounded-md text-center">
            <CardContent className="px-3 text-sm">
              <p className="ff_roman text-sm">
                <span>{viewIntervalCard.arcanaType}</span>{" "}
                <span>{numberToRoman(viewIntervalCard.cardNumber)}</span>
              </p>
              <p className="font-semibold mb-2">
                {viewIntervalCard.nameEn} ({viewIntervalCard.nameKr})
                {viewIntervalCard.reverse ? (
                  <span className="ms-2 inline-flex justify-center items-center  shrink-0 gap-1 px-2 py-0.5 rounded-full text-xs font-medium whitespace-nowrap bg-red-100 text-red-500">
                    역방향
                  </span>
                ) : (
              ...
              <p className="text-xs">
                {viewIntervalCard.keyword
                  ? `${viewIntervalCard.reverseKeyword}`
                  : `${viewIntervalCard.keyword}`}
              </p>
            </CardContent>
          </Card>
