...
export default function GuideTarotCard() {
  ...
  const minor_pentacle = minor.filter(card => card.nameEn.includes('Pentacles'))
          ...
          <>
            <h2 className="text-lg font-semibold mb-2">
              <span className="tossface me-1">
                {SECTION_TITLE.major.icon}
              </span>
              {SECTION_TITLE.major.title}({cardList.length})
            </h2>
            <p className="text-sm text-neutral-600 mb-6">
              메이저 아르카나는 0번 바보(The Fool) 카드부터 21번 세계(The World) 카드까지, <b>인생의 흐름과 중요한 전환점을 상징</b>하는 카드들이에요.
            </p>
          </>
        )
      case "minor":
        return (
          <>
            <h2 className="text-lg font-semibold mb-2">
              <span className="tossface me-1">
                {SECTION_TITLE.minor.icon}
              </span>
              {SECTION_TITLE.minor.title}({cardList.length})
            </h2>
            <p className="text-sm text-neutral-600 mb-6">
              마이너 아르카나는 흙(동전 Pentacles)&middot;물(컵 Cups)&middot;불(지팡이 Wands)&middot;공기(검 Swords), 네 가지 원소를 바탕으로 에이스부터 10까지의 숫자 카드와 왕(King)&middot;여왕(Queen)&middot;기사(Knight)&middot;시종(Page)으로 이루어진 코트 카드로 구성되어있어 <b>일상 속 감정, 관계, 현실적인 상황</b>을 더 섬세하게 보여줘요.
            </p>
          </>
        )
      case "cups":
  ...
  const activeList =
    tab === "major" ? major : tab === "minor" ? minor : all;
